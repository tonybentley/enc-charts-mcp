...
describe('Chart Download and S-57 Parser Integration', () => {
  ...
  describe('Download and Parse San Diego Chart', () => {
    ...
    it('should download a real chart from NOAA', async () => {
      ...
      downloadedFiles = await downloadService.downloadChart(testChartId, (progress) => {
    ...
    it('should parse the downloaded S-57 file', async () => {
      expect(downloadedFiles).toBeDefined();
      expect(downloadedFiles.s57Files.length).toBeGreaterThan(0);
      ...
      const featureTypes = new Set<string>(
        features.features.map(f => f.properties?._featureType as string).filter(Boolean)
      );
    ...
    it('should filter features by type', async () => {
      ...
      const foundTypes = new Set<string | undefined>(
        depthFeatures.features.map(f => f.properties?._featureType as string | undefined)
      );
    ...
    it('should filter depth features by depth range', async () => {
      const s57FilePath = path.join(downloadedFiles.basePath, downloadedFiles.s57Files[0]);
      ...
      shallowFeatures.features.forEach(feature => {
  ...
  describe('Coordinate-based Chart Discovery and Parsing', () => {
    it('should find and parse chart for San Diego coordinates', async () => {
